...
public class KinshipGraphEditor {
	...
	private boolean showUnicode;
	   ...
	   public void buildGraphforAnimalTopDown(String tattoo, int level, 
				boolean founder) {
		   System.out.println("Top Down Kin Tree " + this.showUnicode);
		   this.kinGraph.setAnimalId(tattoo);
		   int yBottom = 240 - y0;
			...
			if (tattoo != null) {
				/*
				if (this.showUnicode)
					nodeLabel = FamilyTreeMgr.getUnicodeForTattoo(tattoo);
				else
				*/
					nodeLabel = tattoo;
				aNode = new AnimalNode(nodeLabel, this.showUnicode);
				kinGraph.add(aNode, new Point(xCenter - 20, yBottom));
			}
			DyadNode[][] ancestors;
			...
			dNode = new DyadNode(d.getDam(),d.getSire(), this.showUnicode);
			ancestors[0][0] = dNode;
			kinGraph.add(dNode, new Point(xCenter - 40, yBottom - hl)); // dyad node offset more
			...
			for (int l=0; l<level-1; l++) {
				for (int n=0; n<ancestors[l].length; n++) {
					...
					/*
					if (this.showUnicode) {
						dam = FamilyTreeMgr.getUnicodeForTattoo(dam);
						sire = FamilyTreeMgr.getUnicodeForTattoo(sire);
					}
					*/
				...
				for (int f=0; f<ancestors[level-1].length; f++) {
					...
					if (dam != null && dam.length() > 0) {
						...
						if (aNode == null) {
							...
							founders[2*f] = aNode;
							kinGraph.add(aNode, new Point(50 + 2*f * 55, 20));
						}
					...
					if (sire != null && sire.length() > 0) {
						...
						if (aNode == null) {
							...
							kinGraph.add(aNode, new Point(50 + (2*f + 1) * 55, 20));
						}
						fLink = new FounderQuickLink();
						fLink.connect(ancestors[level-1][f], aNode);
...
//			
	...
	public void buildGraphforAnimal(String tattoo, int level, 
			boolean founder) {
		   System.out.println("Animal Up Kin Tree " + this.showUnicode);
		this.kinGraph.setAnimalId(tattoo);
		AnimalNode aNode = null;
		if (tattoo != null) {
			aNode = new AnimalNode(tattoo, this.showUnicode);
...
//		
	...
	public KinshipGraph getKinshipGraph() {
		// TODO Auto-generated method stub
		return this.kinGraph;
	}

	public void setCodeChoice(boolean b) {
		System.out.println("Show Unicode set to " + b);
		this.showUnicode = b;
	}
